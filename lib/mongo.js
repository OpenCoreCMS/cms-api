...
function getClient(callback) {
  ...
  return MongoClient.connect(mongoUrl, { useNewUrlParser: true, useUnifiedTopology: true }, (err, client) => {
    if (err || !client) {
      debug('opencorecms:api:error')(`Couldn't connect to Mongo at ${mongoUrl}`);
      if (err) {
        debug('opencorecms:api:error')(err);
      }
      return callback(err);
    }

    debug('opencorecms:api:trace')('Connected to Mongo server');
    mongoClient = client;
  ...
  const rtnModelObject = {
    ...
    insertOne: function insertOne(data, callback) {
      getClient((error, client) => {
        if (error) {
          debug('opencorecms:api:error')(error);
    ...
    find: function find(query, callback) {
      getClient((error, client) => {
        ...
        return col.find(query).toArray((err, reply) => {
          ...
          debug('opencorecms:api:trace')(`Returning ${reply.length} results from ${collectionName}. Query: ${JSON.stringify(query, null, 0)}`);
